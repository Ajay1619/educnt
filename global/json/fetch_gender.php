...
if (
    ...
    try {
        ...
        $result = callProcedure("fetch_pr_gender",$params_procedures);
        if ($result) {
            if ($result['particulars'][0]['status_code'] === 200) {
                $gender_data = [];
                foreach ($result['data'][0] as $key => $value) {
                    $gender_data[] = ['value' => $value['id'], 'title' => $value['title']];
                }
                echo json_encode(['code' => $result['particulars'][0]['status_code'], 'status' => $result['particulars'][0]['status'], 'data' => $gender_data, 'message' => $result['particulars'][0]['message']]);
                exit;
            } else {
