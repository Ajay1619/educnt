...
if (
    ...
    try {
        ...
        $result = callProcedure("fetch_pr_religion",$params_procedures);
      
        if ($result) {
            if ($result['particulars'][0]['status_code'] === 200) {
                $religion_data = [];
                foreach ($result['data'][0] as $key => $value) {
                    $religion_data[] = ['value' => $value['id'], 'title' => $value['title']];
                }
                echo json_encode(['code' => $result['particulars'][0]['status_code'], 'status' => $result['particulars'][0]['status'], 'data' => $religion_data, 'message' => $result['particulars'][0]['message']]);
                exit;
            } else {
