...
<head>
    ...
    <link rel="stylesheet" href="<?= MODULES  . '/faculty_student_attendance/css/student_attendance.css' ?>" />
...
<body>
    ...
    <section id="student-attendance-functions"></section>

    <script>
        const load_student_attendance_functions = () => {
            return new Promise((resolve, reject) => {
                $.ajax({
                    type: 'GET',
                    url: '<?= htmlspecialchars(MODULES . '/faculty_student_attendance/function/faculty_student_attendance_function.php', ENT_QUOTES, 'UTF-8') ?>',
                    headers: {
                        'X-CSRF-Token': '<?= $csrf_token ?>', // Secure CSRF token
                    },
                    success: function(response) {
                        $('#student-attendance-functions').html(response);
        ...
        $(document).ready(async function() {
            try {
                ...
                showLoading();
                await load_student_attendance_functions();
                await loadSidebar();
                await loadTopbar();
                await loadBgCard();
