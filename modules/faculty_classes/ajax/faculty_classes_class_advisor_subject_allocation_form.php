...
if (
    ...
    try {
        ...
        $previous_subject_allocation_id = isset($form_data['previous_subject_allocation_id']) ? sanitizeInput($form_data['previous_subject_allocation_id'], 'int')  : [];
        $subject_id = isset($form_data['subject_id']) ? sanitizeInput($form_data['subject_id'], 'int') : [];
        $room_id = isset($form_data['room_id']) ? sanitizeInput($form_data['room_id'], 'int') : [];
        $faculty_id = isset($form_data['faculty_id']) ? sanitizeInput($form_data['faculty_id'], 'int') : [];
        ...
        $params_procedures = [
            ...
            ['name' => 'subject_id', 'type' => 's', 'value' => json_encode($subject_id)],
            ['name' => 'room_id', 'type' => 's', 'value' => json_encode($room_id)],
            ['name' => 'faculty_id', 'type' => 's', 'value' => json_encode($faculty_id)],
            ['name' => 'previous_subject_allocation_id', 'type' => 's', 'value' => json_encode($previous_subject_allocation_id)],
        ];

        $result = callProcedure("update_pr_class_advisor_subject_allocation", $params_procedures);
        if ($result) {
