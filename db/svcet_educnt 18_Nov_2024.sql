...
--
-- Host: 127.0.0.1
-- Generation Time: Nov 18, 2024 at 05:06 AM
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `fetch_pr_faculty_name_list` (IN `input_dept_id` INT, IN `p_login_id` INT)   BEGIN
    ...
    WHERE 
        f.faculty_status = 1 AND 
        f.faculty_deleted = 0 AND 
        a.account_portal_type= 1 AND 
        a.account_status = 1 AND 
        a.deleted = 0 AND 
        p.prefixes_status = 1 AND 
        p.prefixes_delete = 0 AND 
        (input_dept_id IS NULL OR input_dept_id = 0 OR fo.dept_id = input_dept_id)
    ORDER BY 
        a.account_code;

    -- Insert activity log entry
    CALL insert_user_activity_log(p_login_id, 'svcet_tbl_faculty_personal_info,svcet_tbl_accounts,svcet_tbl_dev_prefixes,svcet_tbl_faculty_official_details', 1);
        -- Return status based on warnings
    IF warning_count > 0 THEN
        SELECT 300 AS status_code, 'warning' AS status, 
               CONCAT('Query executed with ', warning_count, ' warning(s): ', warning_message) AS message;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `fetch_pr_student_table_admission` (IN `p_search_value` VARCHAR(255), IN `p_sort_column` VARCHAR(50), IN `p_order_dir` VARCHAR(4), IN `p_start` INT, IN `p_length` INT, IN `p_admission_type` INT, IN `p_admission_category` INT, IN `p_login_id` INT)   BEGIN
    ...
    SET @filtered_query = CONCAT(
        'SELECT COUNT(*) FROM svcet_tbl_student_personal_info AS spi
         LEFT JOIN svcet_tbl_student_admission_info AS sai ON spi.student_id = sai.student_admission_student_id
         WHERE spi.student_deleted = 0 
         AND sai.admission_deleted = 0 
    ...
    IF p_admission_type > 0 AND p_admission_category > 0 THEN
        EXECUTE stmt_filtered USING @search, @search, p_admission_type, p_admission_category;
    ELSEIF p_admission_type > 0 THEN
        EXECUTE stmt_filtered USING @search, @search, p_admission_type;
    ELSEIF p_admission_category > 0 THEN
        EXECUTE stmt_filtered USING @search, @search, p_admission_category;
    ELSE
        EXECUTE stmt_filtered USING @search, @search;
    END IF;
    ...
    SET @data_query = CONCAT(
        'SELECT spi.student_id, 
                ...
                sai.student_admission_category, 
                sai.admission_status, 
                sed.student_edu_level, 
                sed.student_edu_board, 
                sed.student_edu_degree, 
                sed.student_edu_specialization
        FROM svcet_tbl_student_personal_info AS spi
        LEFT JOIN svcet_tbl_student_admission_info AS sai ON spi.student_id = sai.student_admission_student_id
        LEFT JOIN svcet_tbl_student_education AS sed ON spi.student_id = sed.student_edu_student_id
        WHERE spi.student_deleted = 0 
        AND sai.admission_deleted = 0 
        AND (spi.student_first_name LIKE ? OR spi.student_last_name LIKE ?) ',
        IF(p_admission_type > 0, ' AND sai.student_admission_type = ? ', ''),
    ...
    -- Execute prepared statement for data retrieval with pagination
    IF p_admission_type > 0 AND p_admission_category > 0 THEN
        EXECUTE data_stmt USING @search, @search, p_admission_type, p_admission_category, p_start, p_length;
    ELSEIF p_admission_type > 0 THEN
        EXECUTE data_stmt USING @search, @search, p_admission_type, p_start, p_length;
    ELSEIF p_admission_category > 0 THEN
        EXECUTE data_stmt USING @search, @search, p_admission_category, p_start, p_length;
    ELSE
        EXECUTE data_stmt USING @search, @search, p_start, p_length;
    END IF;
    ...
    -- Insert activity log entry
    CALL insert_user_activity_log(p_login_id, 'svcet_tbl_student_personal_info,svcet_tbl_student_admission_info,svcet_tbl_student_education', 1);
    ...
    IF warning_count > 0 THEN
        SELECT 300 AS status_code, 'warning' AS status, 
               CONCAT('Query executed with ', warning_count, ' warning(s): ', warning_message) AS message;
    ELSE
        SELECT 200 AS status_code, 'success' AS status, 'Student Admission Records Fetched Successfully!' AS message;
    END IF;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `fetch_pr_view_authorities_roles` (IN `p_dept_id` INT, IN `p_login_id` INT)   BEGIN
    -- Declare variables to capture error and warning messages
    DECLARE error_code VARCHAR(5);
    DECLARE error_message VARCHAR(255);
    DECLARE warning_count INT DEFAULT 0;
    ...
    -- Select the data based on the dept_id parameter, if p_dept_id is 0, no department filter is applied
    SELECT 
        f.faculty_official_id,
        f.faculty_id,
        f.dept_id,
        f.faculty_official_group_id,
        d.dept_title AS department_name,
        -- Concatenate faculty name with salutation and middle name
        CONCAT(
            COALESCE(g.general_title, ''), ' ', 
            COALESCE(faculty.faculty_first_name, ''), ' ', 
            COALESCE(faculty.faculty_middle_name, ''), ' ', 
            COALESCE(faculty.faculty_last_name, ''), ' ', 
            COALESCE(faculty.faculty_initial, '')
        ) AS faculty_name,
        -- Fetch profile pic path
        fd.faculty_doc_path AS profile_pic,
        -- Fetch the designation title from the svcet_tbl_dev_general table
        dg.general_title AS designation_title
    FROM 
        svcet_tbl_faculty_official_details f
    LEFT JOIN 
        svcet_tbl_dev_dept d ON f.dept_id = d.dept_id
    LEFT JOIN 
        svcet_tbl_faculty_personal_info faculty ON f.faculty_id = faculty.faculty_id
    LEFT JOIN 
        svcet_tbl_dev_general g ON faculty.faculty_salutation = g.general_id AND g.general_group_id = 19 -- Salutation group
    LEFT JOIN 
        svcet_tbl_faculty_documents fd ON faculty.faculty_id = fd.faculty_doc_faculty_id AND fd.faculty_doc_type = 6 -- Profile pic type
    LEFT JOIN 
        svcet_tbl_dev_general dg ON f.designation = dg.general_id AND dg.general_group_id = 13 -- Designation group
    WHERE 
        f.faculty_official_details_status = 1 -- Only active details
        AND f.faculty_official_deleted = 0 -- Only not deleted
        AND f.faculty_official_group_id IN (1, 2, 3, 4) -- Only group 1, 2, 3, or 4
        AND (p_dept_id = 0 OR f.dept_id = p_dept_id) -- If dept_id is 0, no department filter, else filter by dept_id
    ORDER BY 
        f.faculty_official_group_id, -- Order by group id first
        f.effective_from DESC; -- If group id is the same, order by the most recent effective date (descending)

    -- Insert activity log entry
    CALL insert_user_activity_log(p_login_id, 'svcet_tbl_faculty_official_details,svcet_tbl_dev_dept,svcet_tbl_faculty_personal_info,svcet_tbl_dev_general,svcet_tbl_faculty_documents', 1);
    ...
    IF warning_count > 0 THEN
        SELECT 300 AS status_code, 'warning' AS status, 
               CONCAT('Query executed with ', warning_count, ' warning(s): ', warning_message) AS message;
    ELSE
        SELECT 200 AS status_code, 'success' AS status, 'Authorities Roles Fetched Successfully' AS message;
    END IF;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `fetch_pr_view_class_advisors` (IN `p_dept_id` INT, IN `p_year_of_study_id` INT, IN `p_section_id` INT, IN `p_login_id` INT)   BEGIN
    -- Declare variables for error and warning handling
    DECLARE error_code VARCHAR(5);
    DECLARE error_message VARCHAR(255);
    DECLARE warning_count INT DEFAULT 0;
    ...
    START TRANSACTION;
    SELECT 
        FOD.faculty_official_id,
        FOD.faculty_id,
        FOD.dept_id,
        FOD.year_of_study_id,
        FOD.section_id,
        CONCAT(
            COALESCE(g.general_title, ''), ' ', 
            COALESCE(faculty.faculty_first_name, ''), ' ', 
    ...
    FROM svcet_tbl_faculty_official_details FOD
    LEFT JOIN svcet_tbl_dev_dept D ON FOD.dept_id = D.dept_id AND D.dept_deleted = 0
    LEFT JOIN svcet_tbl_dev_year_of_study YOS ON FOD.year_of_study_id = YOS.year_of_study_id AND YOS.year_of_study_delete = 0
    LEFT JOIN svcet_tbl_dev_section SEC ON FOD.section_id = SEC.section_id AND SEC.section_delete = 0
    LEFT JOIN svcet_tbl_dev_academic_batch AB ON YOS.academic_batch_id = AB.academic_batch_id AND AB.academic_batch_deleted = 0
    LEFT JOIN svcet_tbl_faculty_personal_info faculty ON FOD.faculty_id = faculty.faculty_id
    LEFT JOIN svcet_tbl_dev_general dg ON FOD.designation = dg.general_id AND dg.general_group_id = 13 -- Designation group
    LEFT JOIN svcet_tbl_dev_general g ON faculty.faculty_salutation = g.general_id AND g.general_group_id = 19 -- Salutation group
    WHERE FOD.faculty_official_group_id = 5 
      AND FOD.faculty_official_deleted = 0
      AND (p_dept_id = 0 OR FOD.dept_id = p_dept_id)
      AND (p_year_of_study_id = 0 OR FOD.year_of_study_id = p_year_of_study_id)
      AND (p_section_id = 0 OR FOD.section_id = p_section_id);
    ...
    CALL insert_user_activity_log(
        p_login_id, 
        'svcet_tbl_faculty_official_details,svcet_tbl_dev_dept,svcet_tbl_dev_year_of_study,svcet_tbl_dev_section,svcet_tbl_dev_academic_batch,svcet_tbl_faculty_personal_info,svcet_tbl_dev_general', 
        1
    );
    ...
    IF warning_count > 0 THEN
        SELECT 300 AS status_code, 'warning' AS status, 
               CONCAT('Query executed with ', warning_count, ' warning(s): ', warning_message) AS message;
    ELSE
        SELECT 200 AS status_code, 'success' AS status, 'Class Advisors Fetched Successfully!' AS message;
    END IF;
END$$
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `update_confirmation_student_admission` (IN `p_student_id` INT, IN `p_admission_type` VARCHAR(50))   BEGIN
    -- Declare variables
    DECLARE username_prefix VARCHAR(20) DEFAULT 'SVCET-STU-';
    DECLARE new_account_username VARCHAR(50);
    DECLARE new_account_code INT;
    DECLARE max_account_username INT DEFAULT 0;
    DECLARE max_account_code INT DEFAULT 0;
    DECLARE role_id INT DEFAULT 2;
    DECLARE portal_type INT DEFAULT 2;
    ...
    SET 
        admission_status = 0, -- Assuming 1 is the confirmed status
        student_admission_date = NOW() 
    WHERE 
        student_admission_student_id = p_student_id
        AND admission_status = 1;

    -- Fetch the highest value of account_username
    SELECT MAX(CAST(SUBSTRING(account_username, LENGTH(username_prefix) + 1) AS UNSIGNED))
    INTO max_account_username
    FROM svcet_tbl_accounts
    WHERE account_username LIKE CONCAT(username_prefix, '%');

    -- Generate new account_username
    SET new_account_username = CONCAT(username_prefix, IFNULL(max_account_username, 0) + 1);

    -- Fetch the highest value of account_code
    SELECT MAX(account_code)
    INTO max_account_code
    FROM svcet_tbl_accounts
    WHERE role_id = 2;

    -- Generate new account_code
    SET new_account_code = IFNULL(max_account_code, 0) + 1;

    -- Insert into svcet_tbl_accounts
    INSERT INTO svcet_tbl_accounts (account_username, account_portal_type, account_code, role_id, account_status, deleted)
    VALUES (new_account_username, portal_type, new_account_code, role_id, 1, 0);

    -- Insert into svcet_tbl_student_official_details
    INSERT INTO svcet_tbl_student_official_details (
        student_id, 
        academic_batch_id, 
        student_official_details_status, 
        student_official_details_deleted
    ) 
    VALUES (
        p_student_id, 
        p_admission_type, 
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `update_pr_faculty_skill_profile_info` (IN `p_faculty_id` INT, IN `p_login_id` INT, IN `p_skills` JSON, IN `p_software_skills` JSON, IN `p_interest` JSON, IN `p_languages` JSON)   BEGIN
    ...
    BEGIN
        ...
        GET DIAGNOSTICS CONDITION 1 warning_message = MESSAGE_TEXT;
    END;
    -- Insert core skills (faculty_skill_type = 1)
    SET v_total = JSON_LENGTH(p_skills);
    WHILE v_count < v_total DO
        SET v_skill_name = JSON_UNQUOTE(JSON_EXTRACT(p_skills, CONCAT('$[', v_count, ']')));
        ...
        VALUES (p_faculty_id, 1, v_skill_name, 1, 0);
        SET v_count = v_count + 1;
    END WHILE;

    -- Reset counter for next skill type
    SET v_count = 0;

    -- Insert software skills (faculty_skill_type = 3)
    SET v_total = JSON_LENGTH(p_software_skills);
    WHILE v_count < v_total DO
        SET v_skill_name = JSON_UNQUOTE(JSON_EXTRACT(p_software_skills, CONCAT('$[', v_count, ']')));
        INSERT INTO svcet_tbl_faculty_skills (faculty_skill_faculty_id, faculty_skill_type, faculty_skill_name, faculty_skill_status, faculty_skill_deleted)
        VALUES (p_faculty_id, 3, v_skill_name, 1, 0);
        SET v_count = v_count + 1;
    END WHILE;
    ...
    -- Insert interests (faculty_skill_type = 2)
    SET v_total = JSON_LENGTH(p_interest);
    WHILE v_count < v_total DO
        SET v_skill_name = JSON_UNQUOTE(JSON_EXTRACT(p_interest, CONCAT('$[', v_count, ']')));
        INSERT INTO svcet_tbl_faculty_skills (faculty_skill_faculty_id, faculty_skill_type, faculty_skill_name, faculty_skill_status, faculty_skill_deleted)
        VALUES (p_faculty_id, 2, v_skill_name, 1, 0);
        SET v_count = v_count + 1;
    END WHILE;
    ...
    -- Insert languages (faculty_skill_type = 4)
    SET v_total = JSON_LENGTH(p_languages);
    WHILE v_count < v_total DO
        SET v_skill_name = JSON_UNQUOTE(JSON_EXTRACT(p_languages, CONCAT('$[', v_count, ']')));
        INSERT INTO svcet_tbl_faculty_skills (faculty_skill_faculty_id, faculty_skill_type, faculty_skill_name, faculty_skill_status, faculty_skill_deleted)
        VALUES (p_faculty_id, 4, v_skill_name, 1, 0);
        SET v_count = v_count + 1;
    END WHILE;

    -- Log user activity
    CALL insert_user_activity_log(p_login_id, 'svcet_tbl_faculty_skills', 2); -- action_code = 2 for insert
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `update_update_student_admission_info` (IN `p_student_admission_student_id` INT, IN `p_student_admission_type` TINYINT, IN `p_student_admission_category` TINYINT, IN `p_student_hostel` TINYINT, IN `p_student_admission_know_about_us` INT, IN `p_student_transport` TINYINT, IN `p_student_reference` INT, IN `p_student_admission_reg_no` VARCHAR(50), IN `p_student_course_preference1` INT, IN `p_student_course_preference2` INT, IN `p_student_course_preference3` INT, IN `p_student_concession_subject` INT, IN `p_student_concession_body` TEXT, IN `p_admission_status` TINYINT, IN `p_admission_deleted` TINYINT, IN `p_lateral_entry_year_of_study` INT, IN `p_login_id` INT)   BEGIN
    ...
    IF v_existing_id IS NOT NULL THEN
        ...
        SET
            ...
            admission_status = p_admission_status,
...
INSERT INTO `svcet_tbl_dev_academic_year` (`academic_year_id`, `sem_duration_id`, `academic_batch_id`, `academic_year_title`, `academic_year_status`, `academic_year_deleted`) VALUES
(1, 1, 1, '2023-2024', 1, 0),
(2, 2, 2, '2023-2024', 1, 0),
(3, 3, 3, '2023-2024', 1, 0),
(4, 4, 4, '2023-2024', 1, 0);
...
INSERT INTO `svcet_tbl_dev_dept` (`dept_id`, `sem_duration_id`, `academic_year_id`, `academic_batch_id`, `dept_title`, `dept_short_name`, `dept_status`, `dept_deleted`) VALUES
(1, 1, 1, 1, 'Computer Science and Engineering', 'CSE', 1, 0),
(2, 2, 1, 1, 'Mechanical Engineering', 'MECH', 1, 0),
(3, 3, 1, 1, 'Electrical Engineering', 'EEE', 1, 0),
(4, 4, 1, 1, 'Civil Engineering', 'CIVIL', 1, 0),
(5, 1, 1, 2, 'Information Technology', 'IT', 1, 0),
(6, 2, 1, 2, 'Chemical Engineering', 'CHEM', 1, 0),
(7, 3, 1, 2, 'Aeronautical Engineering', 'AERO', 1, 0),
(8, 4, 1, 2, 'Biotechnology', 'BT', 1, 0),
(9, 1, 1, 3, 'Automobile Engineering', 'AUTO', 1, 0),
(10, 2, 1, 3, 'Robotics', 'ROBO', 1, 0);
...
(25, 1, 'Update SSLC Info ', 2, 1, 1, 'faculty-profile?action=add&route=faculty&type=education&tab=sslc', 0, 1, 0),
...
(33, 1, 'Update Document Upload Info ', 2, 1, 1, 'faculty-profile?action=add&route=faculty&type=documentupload&tab=document', 0, 1, 0),
...
(66, 1, 'Update Course Preference Info ', 2, 1, 2, 'faculty-profile?action=add&route=student&type=course&tab=course', 0, 1, 0),
...
CREATE TABLE `svcet_tbl_dev_section` (
  ...
  `section_status` tinyint(1) NOT NULL DEFAULT 1 COMMENT '1 - Active, 0 - Inactive',
...
CREATE TABLE `svcet_tbl_dev_sem_duration` (
  ...
  `sem_duration_group_id` int(11) NOT NULL,
  `sem_duration_section_id` int(11) NOT NULL,
  `sem_duration_academic_batch_id` int(11) NOT NULL,
  `sem_duration_academic_year_id` int(11) NOT NULL,
  `sem_duration_year_of_study_id` int(11) NOT NULL,
  `sem_duration_sem_id` int(11) NOT NULL,
  `sem_duration_dept_id` int(11) NOT NULL,
...
INSERT INTO `svcet_tbl_dev_sem_duration` (`sem_duration_id`, `sem_duration_title`, `sem_duration_group_id`, `sem_duration_section_id`, `sem_duration_academic_batch_id`, `sem_duration_academic_year_id`, `sem_duration_year_of_study_id`, `sem_duration_sem_id`, `sem_duration_dept_id`, `sem_duration_start_date`, `sem_duration_end_date`, `sem_duration_status`, `sem_duration_delete`) VALUES
(1, 'Odd Sem', 1, 1, 1, 1, 1, 1, 1, '2024-10-15', '2025-01-15', 1, 0),
(2, 'Odd Sem', 2, 2, 2, 2, 2, 2, 1, '2024-10-15', '2025-01-15', 1, 0),
(3, 'Odd Sem', 3, 3, 3, 3, 3, 3, 1, '2024-10-15', '2025-01-15', 1, 0),
(4, 'Odd Sem', 4, 4, 4, 4, 4, 4, 1, '2024-10-15', '2025-01-15', 1, 0);
...
CREATE TABLE `svcet_tbl_dev_year_of_study` (
  ...
  `year_of_study_status` tinyint(1) NOT NULL DEFAULT 1 COMMENT '1 - Active, 0 - Inactive',
...
CREATE TABLE `svcet_tbl_faculty_official_details` (
  ...
  `year_of_study_id` int(11) DEFAULT NULL,
  `section_id` int(11) DEFAULT NULL,
  `faculty_official_group_id` int(11) NOT NULL COMMENT '1 - Principal | 2 - Vice Principal | 3 - Dean | 4 - HODs | 5 - Class Advisors',
...
INSERT INTO `svcet_tbl_faculty_official_details` (`faculty_official_id`, `faculty_id`, `dept_id`, `year_of_study_id`, `section_id`, `faculty_official_group_id`, `effective_from`, `effective_to`, `faculty_official_deleted`, `designation`, `faculty_salary`, `faculty_official_details_status`) VALUES
(1, 11, 1, 1, 1, 1, '2024-11-15 15:18:05', NULL, 0, 167, 50000.00, 1),
(2, 12, 1, 1, 1, 2, '2024-11-15 15:18:05', NULL, 0, 1, 60000.00, 1),
(3, 13, 2, 1, 1, 3, '2024-11-15 15:18:05', NULL, 0, 1, 40000.00, 1),
(4, 14, 2, 1, 1, 4, '2024-11-15 15:18:05', NULL, 0, 1, 52000.00, 1),
(5, 15, 3, 1, 1, 4, '2024-11-15 15:18:05', NULL, 0, 1, 75000.00, 1),
(6, 16, 3, 1, 1, 4, '2024-11-15 15:18:05', NULL, 0, 1, 80000.00, 1),
(7, 17, 1, 1, 1, 4, '2024-11-15 15:18:05', NULL, 0, 1, 58000.00, 1),
(8, 18, 1, 1, 1, 5, '2024-11-15 15:18:05', NULL, 0, 1, 62000.00, 1),
(9, 19, 2, 1, 1, 0, '2024-11-15 15:18:05', NULL, 0, 1, 45000.00, 1),
(10, 17, 2, 1, 1, 0, '2024-11-15 15:18:05', NULL, 0, 1, 70000.00, 1),
(11, 1027, 1, NULL, NULL, 0, '2024-11-15 15:18:05', NULL, 0, 167, NULL, 1),
(12, 1028, 1, NULL, NULL, 0, '2024-11-15 15:18:05', NULL, 0, 167, NULL, 1),
(13, 1029, NULL, NULL, NULL, 0, '2024-11-15 15:18:05', NULL, 0, NULL, NULL, 1);
...
(69, 11, 1, 264, 1, '2024-11-09', '2024-11-09', 0, 0),
(71, 17, 1, 264, 2, '2024-11-09', '2024-11-09', 0, 0),
(72, 11, 1, 264, 2, '2024-11-09', '2024-11-09', 0, 0),
(73, 1028, 1, 264, 2, '2024-11-09', '2024-11-09', 0, 0),
(74, 17, 1, 264, 1, '2024-11-09', '2024-11-09', 0, 0),
(75, 11, 1, 264, 2, '2024-11-09', '2024-11-09', 0, 0),
(76, 17, 1, 264, 1, '2024-11-09', '2024-11-09', 0, 0),
(77, 11, 1, 264, 1, '2024-11-09', '2024-11-09', 0, 0),
(78, 11, 1, 264, 1, '2024-11-09', '2024-11-09', 0, 0),
(79, 1028, 1, 264, 2, '2024-11-09', NULL, 1, 0),
(80, 11, 1, 265, 4, '2024-11-09', NULL, 1, 0),
(81, 17, 1, 265, 2, '2024-11-09', '2024-11-09', 0, 0);
...
CREATE TABLE `svcet_tbl_faculty_skills` (
  ...
  `faculty_skill_type` tinyint(1) DEFAULT NULL COMMENT '1 - Core Expertise | 2 - Interest | 3 - Software Skill | 4 - Language Type of skill: core, interest, software, or language',
...
(8, 11, 4, '', 1, 0),
(9, 11, 1, 'coding', 1, 0),
(10, 11, 1, 'development', 1, 0),
(11, 11, 3, 'programming language', 1, 0),
(12, 11, 2, 'cooking', 1, 0),
(13, 11, 4, 'tamil', 1, 0),
(14, 11, 1, 'coding', 1, 0),
(15, 11, 1, 'development', 1, 0),
(16, 11, 3, 'programming language', 1, 0),
(17, 11, 2, 'cooking', 1, 0),
(18, 11, 4, 'English', 1, 0),
(19, 11, 1, 'ada', 1, 0),
(20, 11, 1, 'aad', 1, 0),
...
(43, 11, 1, 'hi', 1, 0),
(44, 11, 1, 'hi', 1, 0),
(45, 1028, 1, 'Coding', 1, 0),
(46, 1028, 1, 'Software development', 1, 0),
(47, 1028, 3, 'html', 1, 0),
(48, 1028, 3, 'css', 1, 0),
(49, 1028, 3, 'php', 1, 0),
(50, 1028, 3, 'javacsript', 1, 0),
(51, 1028, 2, 'Dance', 1, 0),
(52, 1028, 4, 'English', 1, 0),
(53, 1028, 4, 'Tamil', 1, 0),
(54, 1028, 1, 'Coding', 1, 0),
(55, 1028, 1, 'Software development', 1, 0),
(56, 1028, 3, 'html', 1, 0),
(57, 1028, 3, 'css', 1, 0),
(58, 1028, 3, 'php', 1, 0),
(59, 1028, 3, 'javacsript', 1, 0),
(60, 1028, 2, 'Dance', 1, 0),
(61, 1028, 4, 'English', 1, 0),
(62, 1028, 4, 'Tamil', 1, 0);
...
CREATE TABLE `svcet_tbl_student_admission_info` (
  ...
  `admission_status` tinyint(1) DEFAULT 0 COMMENT '0 - Pending | 1 - Active | 2 - Inactive | 3 - Discontinued',
...
ALTER TABLE `svcet_tbl_dev_academic_year`
  ...
  ADD KEY `svcet_tbl_dev_academic_year_ibfk_2` (`sem_duration_id`);
...
ALTER TABLE `svcet_tbl_dev_dept`
  ...
  ADD KEY `academic_batch_id` (`academic_batch_id`),
  ADD KEY `svcet_tbl_dev_dept_ibfk_3` (`sem_duration_id`);
...
ALTER TABLE `svcet_tbl_dev_sem_duration`
  ...
  ADD KEY `sem_duration_dept_id` (`sem_duration_dept_id`),
  ADD KEY `sem_duration_group_id` (`sem_duration_group_id`),
  ADD KEY `sem_duration_section_id` (`sem_duration_section_id`),
  ADD KEY `sem_duration_academic_batch_id` (`sem_duration_academic_batch_id`),
  ADD KEY `sem_duration_academic_year_id` (`sem_duration_academic_year_id`),
  ADD KEY `sem_duration_year_of_study_id` (`sem_duration_year_of_study_id`),
  ADD KEY `sem_duration_sem_id` (`sem_duration_sem_id`);
...
--
ALTER TABLE `svcet_tbl_dev_pages`
  MODIFY `page_id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=85;
...
--
ALTER TABLE `svcet_tbl_dev_role_permission`
  MODIFY `role_perm_id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=240;
...
--
ALTER TABLE `svcet_tbl_faculty_official_details`
  MODIFY `faculty_official_id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=14;
...
--
ALTER TABLE `svcet_tbl_faculty_roles_and_responsibilities`
  MODIFY `faculty_roles_and_responsibilities_id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=82;
...
--
ALTER TABLE `svcet_tbl_faculty_skills`
  MODIFY `faculty_skill_id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=63;
...
--
ALTER TABLE `svcet_tbl_student_education`
  MODIFY `student_edu_id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=24;
...
--
-- Constraints for table `svcet_tbl_dev_academic_year`
--
ALTER TABLE `svcet_tbl_dev_academic_year`
  ADD CONSTRAINT `svcet_tbl_dev_academic_year_ibfk_1` FOREIGN KEY (`academic_batch_id`) REFERENCES `svcet_tbl_dev_academic_batch` (`academic_batch_id`),
  ADD CONSTRAINT `svcet_tbl_dev_academic_year_ibfk_2` FOREIGN KEY (`sem_duration_id`) REFERENCES `svcet_tbl_dev_sem_duration` (`sem_duration_id`);

--
-- Constraints for table `svcet_tbl_dev_dept`
--
ALTER TABLE `svcet_tbl_dev_dept`
  ADD CONSTRAINT `svcet_tbl_dev_dept_ibfk_1` FOREIGN KEY (`academic_year_id`) REFERENCES `svcet_tbl_dev_academic_year` (`academic_year_id`),
  ADD CONSTRAINT `svcet_tbl_dev_dept_ibfk_2` FOREIGN KEY (`academic_batch_id`) REFERENCES `svcet_tbl_dev_academic_batch` (`academic_batch_id`),
  ADD CONSTRAINT `svcet_tbl_dev_dept_ibfk_3` FOREIGN KEY (`sem_duration_id`) REFERENCES `svcet_tbl_dev_sem_duration` (`sem_duration_id`);
...
--
-- Constraints for table `svcet_tbl_dev_sem_duration`
--
ALTER TABLE `svcet_tbl_dev_sem_duration`
  ADD CONSTRAINT `svcet_tbl_dev_sem_duration_ibfk_1` FOREIGN KEY (`sem_duration_dept_id`) REFERENCES `svcet_tbl_dev_dept` (`dept_id`),
  ADD CONSTRAINT `svcet_tbl_dev_sem_duration_ibfk_2` FOREIGN KEY (`sem_duration_group_id`) REFERENCES `svcet_tbl_dev_group` (`group_id`),
  ADD CONSTRAINT `svcet_tbl_dev_sem_duration_ibfk_3` FOREIGN KEY (`sem_duration_section_id`) REFERENCES `svcet_tbl_dev_section` (`section_id`),
  ADD CONSTRAINT `svcet_tbl_dev_sem_duration_ibfk_4` FOREIGN KEY (`sem_duration_academic_batch_id`) REFERENCES `svcet_tbl_dev_academic_batch` (`academic_batch_id`),
  ADD CONSTRAINT `svcet_tbl_dev_sem_duration_ibfk_5` FOREIGN KEY (`sem_duration_academic_year_id`) REFERENCES `svcet_tbl_dev_academic_year` (`academic_year_id`),
  ADD CONSTRAINT `svcet_tbl_dev_sem_duration_ibfk_6` FOREIGN KEY (`sem_duration_year_of_study_id`) REFERENCES `svcet_tbl_dev_year_of_study` (`year_of_study_id`),
  ADD CONSTRAINT `svcet_tbl_dev_sem_duration_ibfk_7` FOREIGN KEY (`sem_duration_sem_id`) REFERENCES `svcet_tbl_dev_sem` (`sem_id`);
...
ALTER TABLE `svcet_tbl_faculty_official_details`
  ...
  ADD CONSTRAINT `svcet_tbl_faculty_official_details_ibfk_6` FOREIGN KEY (`year_of_study_id`) REFERENCES `svcet_tbl_dev_year_of_study` (`year_of_study_id`),
  ADD CONSTRAINT `svcet_tbl_faculty_official_details_ibfk_8` FOREIGN KEY (`section_id`) REFERENCES `svcet_tbl_dev_section` (`section_id`);
...
CREATE TABLE svcet_tbl_faculty_authorities (
    faculty_authorities_id INT AUTO_INCREMENT PRIMARY KEY,
    faculty_id INT,
    faculty_authorities_group_id INT DEFAULT null COMMENT '1 - Principal | 2 - Vice Principal | 3 - Dean | 4 - HODs | 5 - Exam Cell Head | 6 - Admission Cell Head | 7 - Placement Cell Head',
    dept_id INT DEFAULT NULL,
    effective_from TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    effective_to TIMESTAMP DEFAULT null,
    faculty_authorities_status INT DEFAULT 1 COMMENT '1 - Active | 2 - Inactive | 3 - Completed',
    faculty_authorities_deleted INT DEFAULT 0 COMMENT '0 - Not Deleted | 1 - Deleted',
    FOREIGN KEY (faculty_id) REFERENCES svcet_tbl_faculty_personal_info(faculty_id)
    FOREIGN KEY (dept_id) REFERENCES svcet_tbl_dev_dept(dept_id)
);

CREATE TABLE svcet_tbl_faculty_class_advisors (
    faculty_class_advisors_id INT AUTO_INCREMENT PRIMARY KEY,
    faculty_id INT,
    year_of_study_id INT,
    section_id INT,
    effective_from TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    effective_to TIMESTAMP DEFAULT null,
    faculty_class_advisors_status INT DEFAULT 1 COMMENT '1 - Active | 2 - Inactive | 3 - Completed',
    faculty_class_advisors_deleted INT DEFAULT 0 COMMENT '0 - Not Deleted | 1 - Deleted',
    FOREIGN KEY (faculty_id) REFERENCES svcet_tbl_faculty_personal_info(faculty_id)
    FOREIGN KEY (year_of_study_id) REFERENCES svcet_tbl_dev_year_of_study(year_of_study_id)
    FOREIGN KEY (section_id) REFERENCES svcet_tbl_dev_section(section_id)
);

CREATE TABLE svcet_tbl_faculty_mentor (
    faculty_mentor_id INT AUTO_INCREMENT PRIMARY KEY,
    faculty_id INT,
    student_id INT,
    effective_from TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    effective_to TIMESTAMP DEFAULT null,
    faculty_mentor_status INT DEFAULT 1 COMMENT '1 - Active | 2 - Inactive | 3 - Completed',
    faculty_mentor_deleted INT DEFAULT 0 COMMENT '0 - Not Deleted | 1 - Deleted',
    FOREIGN KEY (faculty_id) REFERENCES svcet_tbl_faculty_personal_info(faculty_id)
    FOREIGN KEY (student_id) REFERENCES svcet_tbl_student_personal_info(student_id)
);